...
export default async function handler(req, res) {
  ...
  try {
    ...
    const [pairsRes, kalshiRes, polyRes] = await Promise.all([
      ...
      // Kalshi IDs always start with "KX" — use ilike to filter correctly
      supabase.from("markets").select("id, title, yes_price, no_price, volume, sport_tag, event_ticker, side_label, close_time").ilike("id", "KX%").in("sport_tag", tags),
      // Polymarket IDs are numeric strings — filter by slug existing
      supabase.from("markets").select("id, title, yes_price, no_price, volume, slug, side_label, outcomes, outcome_prices").not("id", "ilike", "KX%"),
    ]);
