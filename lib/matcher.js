...
// WHY DIDN'T THESE TWO MATCH?
//
// Search is the review queue — what a reader looks up and finds
// unmatched is the demand signal for this matcher — but "0 matched" on
// nine live markets says nothing about WHICH of the three things went
// wrong: the score was under the floor, a gate rejected the pair, or
// the counterparty was taken by a better-scoring Kalshi row. Reading a
// thousand-line topScores dump to answer that for ONE market is not a
// workflow anybody will repeat.
//
// This scores one Kalshi row against every Polymarket row in its
// category and reports the score AND the gate verdict per candidate,
// so the answer is a single URL. It runs the same cosineSimilarity and
// the same scalarSignaturesCompatible the matcher does, rather than a
// second copy that could tell you a comforting lie.
export function explainKalshiRow(km, polyDb, threshold, limit = 10) {
  const kVec = parseVector(km.embedding_v);
  if (!kVec) return { error: "kalshi row has no usable embedding_v" };

  const scored = [];
  for (const pm of (polyDb || [])) {
    if (km.sport_tag !== pm.sport_tag) continue;
    const vec = parseVector(pm.embedding_v);
    if (!vec) continue;
    const score = cosineSimilarity(kVec, vec);
    const gate = scalarSignaturesCompatible(km.title, pm.title, km.sport_tag);
    scored.push({
      score: Math.round(score * 10000) / 10000,
      polyId: String(pm.id),
      poly: pm.title,
      gate: gate ? "pass" : "REJECTED",
      // The two reasons are independent and a candidate can fail both,
      // so they are reported separately rather than as one verdict.
      overThreshold: score >= threshold,
      wouldPair: score >= threshold && gate,
    });
  }
  scored.sort((a, b) => b.score - a.score);
  return {
    kalshiId: String(km.id),
    kalshi: km.title,
    sportTag: km.sport_tag,
    threshold,
    polyCompared: scored.length,
    wouldPair: scored.filter(c => c.wouldPair).length,
    // The best candidate REGARDLESS of threshold and gate, which is the
    // one that says whether there is anything close at all.
