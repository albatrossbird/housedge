...
//
// polymarket.com has the SAME split — see polymarketComUrl below.
// `eventSlug` is the parent event, stored as markets.event_ticker. A
// non-game .us market is addressed by its EVENT, not by itself:
// /event/gdpc-us-saa-q3-2026-10-29-gt2pt0 is a not-found page, while
// /event/us-saa-q3-2026-10-29?marketSlug=gdpc-us-saa-q3-2026-10-29-gt2pt0
// is the market. It cannot be derived — the family prefix varies in
// length and "cpc-btc-100k-10-31-2026" belongs to the event "btc-100k",
// which drops the date.
export function polymarketUsUrl(slug, eventSlug = null) {
  const s = String(slug || "").trim();
  const ev = String(eventSlug || "").trim();
  if (!s && !ev) return "https://polymarket.us/";
  if (!s) return `https://polymarket.us/event/${ev}`;

  if (!s.startsWith("aec-")) {
    // Without the parent this is the market slug on an /event/ route,
    // which 404s — but it is still the closest thing we have, and it
    // was the behaviour before the event slug was stored. Rows written
    // before that backfill land here.
    if (!ev) return `https://polymarket.us/event/${s}`;
    return ev === s
      ? `https://polymarket.us/event/${ev}`
      : `https://polymarket.us/event/${ev}?marketSlug=${encodeURIComponent(s)}`;
  }
...
// A polymarket.com web URL.
//
// The same split as .us, and it was wrong here for every game on the
// site: `/event/mlb-ari-hou-2026-09-04` renders a not-found page while
// `/sports/mlb/mlb-ari-hou-2026-09-04` is the game. The comment above
// used to claim .com was unaffected; it was not, and nobody checked
// because the .us fix looked like the whole problem.
//
// Unlike .us there is no `aec-` prefix to key on — the event slug and
// the market slug are the SAME string on .com (verified against Gamma:
// /markets?slug=mlb-ari-hou-2026-09-04 reports an event of the same
// slug). So a game is recognised by its SHAPE: league, two team codes,
// and an ISO date. `mlb-world-series-champion-2026` does not match,
// which is correct — futures markets do live under /event/.
//
// STATUS CODES CANNOT TEST THIS, and neither can the not-found marker:
// polymarket.com answers 200 for every path, and the string "not found"
// appears in the bundle of pages that ARE found. The discriminator is
// content that only the right page has — the built URL returns 18
// mentions of the two teams where /event/ returns zero, against a
// deliberately-wrong control that also returns zero.
const COM_GAME_SLUG = /^([a-z]{2,10})-[a-z0-9]{2,4}-[a-z0-9]{2,4}-\d{4}-\d{2}-\d{2}$/;

export function polymarketComUrl(slug) {
  const s = String(slug || "").trim();
  if (!s) return "https://polymarket.com/";
  const m = s.match(COM_GAME_SLUG);
  return m
    ? `https://polymarket.com/sports/${m[1]}/${s}`
