...
const TOUCH_RE = /\bwhen will\b|\b(?:hits?|reach(?:es)?|cross(?:es)?|touch(?:es)?)\b|\b(?:dips?|drops?|falls?) to\b|\bhow (?:high|low)\b|\b(?:above|below)\b[^?]*\bby\b/i;
const TERMINAL_RE = /\bprice\s+on\s+\w+\.?\s+\d{1,2},?\s*\d{4}/i
  ;
const TERMINAL_RE2 = /\bprice\s+(?:at|on)\s+the\s+end\s+of\b|\bprice\s+range\s+eoy\b/i;

export function settlementStyle(title) {
  const t = String(title || "");
  const terminal = TERMINAL_RE.test(t) || TERMINAL_RE2.test(t);
  const touch = TOUCH_RE.test(t);
  // "BTC price on Jan 1, 2027? - 150,000 or above" reads as both under
  // these patterns; the explicit moment wins, because that is the part
  // that decides how it settles.
...
export function extractDeadline(title) {
  ...
  m = t.match(/\b(?:by|before|on|at|for)\s+(?:the\s+)?(jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)[a-z]*\.?\s+(\d{1,2}),?\s*(\d{4})/);
...
const BARE_MONTH_DAY = /\b(?:by|before|on|for)\s+(?:the\s+)?(?:jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)[a-z]*\.?\s+\d{1,2}\b/;
...
const BARE_MD_RE = /\b(?:by|before|on|for)\s+(?:the\s+)?(jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)[a-z]*\.?\s+(\d{1,2})\b/;
