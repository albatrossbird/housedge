...
const POLITICAL_ACTS = [
  ...
  ["announce_run",/\bannounce[sd]? (?:a |their )?(?:presidential |white house )?run\b|\bpresidential run\b|\bdeclare[sd]? (?:a )?candidacy\b/],
  ["win_election",/\bwin(?:s)?\b|\bwinner\b|\belection winner\b|\bnomination\b|\bprimary\b/],
...
export function politicalActs(title) {
  const t = String(title || "").toLowerCase();
  return POLITICAL_ACTS.filter(([, re]) => re.test(t)).map(([name]) => name);
}
...
const NAME_RE = /\b([A-Z][a-z]+(?:\s+(?:[A-Z][a-z]+|[A-Z]\.|de|van|von|bin))+)\b/g;
