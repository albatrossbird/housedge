...
export function extractDeadline(title) {
  ...
  // "before 2027" — the boundary, i.e. Jan 1
  m = t.match(/\bbefore\s+(\d{4})\b/);
  if (m) return { y: +m[1], mo: 1, d: 1 };

  // "by 2027" / "in 2026" — end of that year
  m = t.match(/\b(?:by|in)\s+(?:the\s+end\s+of\s+)?(\d{4})\b/);
  if (m) return { y: +m[1], mo: 12, d: 31 };
...
export function deadlinesCompatible(titleA, titleB, toleranceDays = 3) {
  ...
  if (!a || !b) return true; // unparsed on one side: don't block
