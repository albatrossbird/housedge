...
export function extractNumericClaim(title) {
  ...
  // Seat, turnout and governorship buckets. Both venues slice the same
  // quantity into bands and neither slices it the same way: Kalshi's
  // "win 198-202 seats" against Polymarket's "hold between 195 and 199
  // House seats" scored 0.869 and is three seats out, which is the
  // whole distance between a majority and not. Nine of those went
  // through because a band with no "%" in it parsed to nothing.
  //
  // Read only where the title actually names the thing being counted,
  // and read the BUCKET LABEL where there is one — polymarket.us puts
  // it after an em-dash, and the question half carries a year that
  // would otherwise be picked up as the count.
  const seats = extractSeatClaim(title);
  ...
  if (m) {
    ...
    // "more than 1500 billion DOLLARS" is a strike written the long
    // way. Reading it as a dollar amount rather than discarding it is
    // what lets it compare against polymarket.us's "$1.5T" — otherwise
    // the Kalshi side parses to nothing, and a pair with no signature
    // on one side is accepted without ever being checked.
    if (/^\s*(?:dollars?|usd)\b/.test(after) && !/\$/.test(t)) {
      ...
      const down = USD_DOWN.test(t), up = USD_UP.test(t);
      if (isFinite(value) && down !== up) return { unit: "usd", op: up ? "gte" : "lte", value };
    }
...
const SEAT_SUBJECT = /\bseats?\b|\bturnout\b|\bgovernorships?\b|\bgovernors?\b/;

// Kalshi closes a band one tick below the boundary — "100 to 104.99
// million" is the bucket Polymarket writes as "100-105 Million", the
// same convention as "above $99,999.99" being the $100,000 market.
// Normalising rather than tolerating keeps the comparison exact, so
// 115-119.99 against 110-115 still fails.
const tick = v => (Math.round(v * 100) % 100 === 99 ? Math.round(v * 100 + 1) / 100 : v);

export function extractSeatClaim(title) {
  const raw = String(title || "");
  if (!SEAT_SUBJECT.test(raw.toLowerCase())) return null;

  // "the difference between the number of Republican governors and the
  // number of Democratic governors" is not a count of either.
  if (/\bdifference between\b/i.test(raw)) return { unit: "seat_diff", op: "eq", value: 0 };

  // Prefer the bucket label; fall back to the whole title. Years are
  // stripped either way — "2026 Midterms: House Turnout" is not a count.
  const parts = raw.split(/[—–]/);
  const scale = /\bmillion\b/i.test(raw) ? 1e6 : 1;
  const bare = seg => seg.replace(/\b(?:19|20)\d{2}\b/g, " ").replace(/\b\d+th\b/g, " ");

  for (const seg of (parts.length > 1 ? [bare(parts[parts.length - 1]), bare(raw)] : [bare(raw)])) {
    const t = seg.toLowerCase();
    let m;
    m = t.match(/between\s+(\d[\d,]*(?:\.\d+)?)\s*(?:million\s*)?and\s+(\d[\d,]*(?:\.\d+)?)/) ||
        t.match(/(\d[\d,]*(?:\.\d+)?)\s*(?:million\s*)?(?:to|through|[-–—])\s*(\d[\d,]*(?:\.\d+)?)/);
    if (m) {
      const low = parseFloat(m[1].replace(/,/g, "")), high = parseFloat(m[2].replace(/,/g, ""));
      if (isFinite(low) && isFinite(high) && high >= low) {
        return { unit: "seats", op: "range", low: tick(low) * scale, high: tick(high) * scale };
      }
    }
    // Seats are whole things, so a strict comparator resolves to the
    // next one: "more than 12 seats" IS "13 or more". Leaving it as an
    // inclusive 12 made it compare equal to Polymarket's "12+", which
    // is one bucket below and a different bet — and the shared gt/gte
    // grouping downstream would never have separated them.
    m = t.match(/(\d[\d,]*)\s*(?:or more|\+|or above|or higher)/) || t.match(/at least\s+(\d[\d,]*)/);
    if (m) return { unit: "seats", op: "gte", value: parseFloat(m[1].replace(/,/g, "")) * scale };
    m = t.match(/(?:more than|above|over)\s+(\d[\d,]*)/);
    if (m) return { unit: "seats", op: "gte", value: (parseFloat(m[1].replace(/,/g, "")) + 1) * scale };

    m = t.match(/(\d[\d,]*)\s*(?:or fewer|or less|or below)/);
    if (m) return { unit: "seats", op: "lte", value: parseFloat(m[1].replace(/,/g, "")) * scale };
    m = t.match(/(?:fewer than|less than|below|under)\s+(\d[\d,]*)/);
    if (m) return { unit: "seats", op: "lte", value: (parseFloat(m[1].replace(/,/g, "")) - 1) * scale };

    m = t.match(/exactly\s+(\d[\d,]*)/) || t.match(/^\s*(\d[\d,]*)\s*$/);
    if (m) return { unit: "seats", op: "eq", value: parseFloat(m[1].replace(/,/g, "")) * scale };
  }
  return null;
}
...
export function numericClaimsCompatible(a, b) {
  ...
  if (a.op === "range" || b.op === "range") {
    ...
    // Seat and turnout bands are already normalised to their boundary,
    // so they compare exactly on both edges; the low-edge tick of slack
    // is a fact about Kalshi's PERCENT labels, not about counts.
    if (a.unit === "seats" || b.unit === "seats") {
  ...
  if (a.unit === "usd" || a.unit === "usd_race" || a.unit === "quantity" || a.unit === "seats") {
...
// Polymarket counts members who RETIRE. Three pairs in one politics run
// crossed those, and the prices say how different they are: "exactly 6
// House Republicans lose their primary" was matched against "the number
// who retire is 44 OR MORE". Same chamber, same party, same year, and
// not remotely the same question.
//
// The numeric gate does not save this. It rejects only when BOTH sides
// parse, and "be 44 or more?" is a phrasing extractNumericClaim does not
// read — so two counts that disagree by a factor of seven sailed
// through on embedding score alone. Parsing that phrasing would help,
// but it would not FIX this: six retirements and six primary losses are
// different markets even when the number agrees.
//
// Compared as sets and rejected only on DISJOINTNESS, like econMetrics:
// a title naming both modes, or naming none, blocks nothing. Scoped to
// counting-people markets by requiring both sides to name a mode at
// all, which keeps it inert on the ordinary "will X win" races that are
// most of this category.
const DEPARTURE_MODES = [
  ["retire", /\bretir(?:e|es|ed|ing|ement)\b|\bnot seek re-?election\b|\bstep(?:s|ping)? down\b/],
  ["lose_primary", /\blos(?:e|es|ing|t)\b[^.?]{0,40}\bprimar(?:y|ies)\b|\bprimar(?:y|ies)\b[^.?]{0,20}\bdefeat/],
  ["lose_general", /\blos(?:e|es|ing|t)\b[^.?]{0,40}\b(?:re-?election|general election|their seat)\b/],
];

export function departureModes(title) {
  const t = String(title || "").toLowerCase();
  const found = DEPARTURE_MODES.filter(([, re]) => re.test(t)).map(([name]) => name);
  // Losing a primary IS losing an election, and "lose their primary"
  // matches both patterns. The specific one is the claim, so the
  // general tag goes or the sets would still intersect.
  return found.includes("lose_primary") ? found.filter(m => m !== "lose_general") : found;
}

function departuresCompatible(titleA, titleB) {
  const a = departureModes(titleA), b = departureModes(titleB);
...
const POLITICAL_ACTS = [
  ...
  // Kalshi runs endorsement markets in the same words as the race:
  // "Will Donald Trump ENDORSE J.D. Vance in the 2028 presidential
  // election before Mar 1, 2028?" against "Will JD Vance WIN the 2028
  // US Presidential Election?" scored 0.891. Being endorsed is not
  // winning, and the endorser's name is not the subject.
  ["endorse",      /\bendorse[sd]?\b|\bendorsement\b/],
  // "How many House seats will the Democrats win in Texas?" is a count
  // across a state; "Will the Democratic Party win the UT-02 House
  // seat?" is one district; "Who will win the Texas House of
  // Representatives?" is control of the chamber. All three score 0.86+
  // against each other because they share every word that carries
  // similarity and differ only in what is being counted.
  ["seat_count",   /\bhow many\b[\s\S]*\bseats?\b|\b(?:exactly|more than|at least|fewer than|between)\s+\d[\s\S]{0,30}?\bseats?\b|\bwin\s+\d+\s+seats?\b|\b\d+\s*[-–]\s*\d+\s+seats?\b|\bhold\s+(?:exactly\s+)?\d[\s\S]{0,30}?\bseats?\b/],
  ["join_admin",   /\brejoin\b|\bjoin the [a-z]+ administration\b|\bbe appointed\b/],
  // Winning the national popular vote is not holding a number of
  // seats — the 2026 House pair proves it, since a party can take the
  // vote and lose the chamber.
  ["popular_vote", /\bpopular vote\b/],
...
const ACT_DOMINATES = {
  ...
  endorse:         ["win_election", "nomination_race", "primary_race"],
  ...
  seat_count:      ["win_election"],
  popular_vote:    ["win_election", "seat_count"],
  join_admin:      ["win_election", "nomination_race"],
...
// Unicode classes rather than enumerated Latin-1 ranges: "Stabilitātei"
// carries a U+0101, outside à-ÿ, and so did not read as a word at all.
//
// The body of a name is \p{L}, not \p{Ll}: a surname can carry an
// INTERNAL capital. Requiring lowercase after the first letter stops
// "McBath" at "Mc", and with no trailing boundary to reject the
// fragment, "Lucy McBath" read as the name "Lucy Mc" — which shares no
// token with "the Democratic Party" and rejected twenty-one correct
// district pairs. The old pattern survived this only by accident: its
// trailing \b failed on the fragment, so the name came back empty and
// the gate never fired at all.
const NAME_RE = /(\p{Lu}[\p{L}'’-]*(?:\s+(?:\p{Lu}[\p{L}'’-]*|\p{Lu}\.|de|van|von|bin))+)/gu;
...
export function properNames(title) {
  ...
  for (const m of t.matchAll(NAME_RE)) {
    ...
    // A party is not a person, and partiesNamed already compares party
    // identity. Kalshi labels a district market with its CANDIDATE
    // ("— Lucy McBath") where Polymarket names the PARTY ("Will the
    // Democratic Party win the GA-06 House seat?"); those are the same
    // claim at two levels of description, and treating both as names
    // makes them disagree. Twenty-one correct district pairs had been
    // surviving only because "McBath" failed to parse at all.
    if (/\bpart(?:y|ies)$/.test(low)) continue;
...
export function officesNamed(title) {
  const t = String(title || "").toLowerCase();
  const out = new Set();
  // "White House" must not read as the chamber, so the bare noun is
  // only accepted in the phrasings that can only mean the chamber.
  if (/\bhouse (?:race|seat|of representatives)\b|\bhouse seats?\b|\bcongressional district\b|\b(?:win|control|hold|flip)\s+the\s+house\b/.test(t)) out.add("house");
  // An assembly IS the lower chamber, so it reads as "house" rather
  // than as an office of its own — a state-assembly pair still matches
  // a state-assembly pair, while "Wisconsin State Assembly" against
  // "the Wisconsin GOVERNOR race" now disagrees on office instead of
  // stating no office at all and blocking nothing.
  if (/\bstate assembly\b|\bgeneral assembly\b/.test(t)) out.add("house");
  if (/\bsenate\b|\bsenator\b|\bsenatorial\b/.test(t)) out.add("senate");
  if (/\bgovernor(?:ship)?\b|\bgubernatorial\b/.test(t)) out.add("governor");
  // "presidency" does not contain "president" — it is presiden-CY — so
  // a pattern built on the word "president" reads no office at all on
  // "nominee for the Presidency" or "for the Vice Presidency", and the
  // VP nomination paired with the presidential one.
  if (/\bvice[- ]presiden(?:t|tial|cy)/.test(t)) out.add("vp");
  else if (/\bpresiden(?:t|ts|tial|cy)\b/.test(t)) out.add("president");
  if (/\bmayor(?:al)?\b/.test(t)) out.add("mayor");
  if (/\bprime minister\b/.test(t)) out.add("pm");
  if (/\bbundestag\b|\bknesset\b|\bparliament(?:ary)?\b|\bcongress\b/.test(t)) out.add("legislature");
  if (/\battorney general\b/.test(t)) out.add("ag");
  if (/\bsecretary of state\b/.test(t)) out.add("sos");
  if (/\blieutenant governor\b/.test(t)) out.add("ltgov");
  return [...out];
}
...
const PLACEHOLDER_RE =
  /\bwill\s+(?:the\s+)?(?:candidate|person|leader|option|team|party)\s+[a-z0-9]{1,3}\b|\bwill\s+[a-z]\s+win\b|\bwill\s+other\s+(?:be|win)\b/i;
...
export function isCombination(title) {
  ...
  if (/\band\b[\s\S]*\bbe the\b[\s\S]*\b(?:nominees|tickets?)\b/.test(t)) return true;
  return officesNamed(title).length >= 2 && /\band\b/.test(t);
}

// A national contest is not a regional one. "Will Greens win the next
// election to the German BUNDESTAG?" against "Will Grüne win the most
// seats in the 2026 BERLIN STATE elections?" scored 0.879 — same party,
// same country, same year, different election. statesNamed only knows
// US states, so nothing else separates them.
export function governmentLevel(title) {
  const t = String(title || "").toLowerCase();

  // A STATE CHAMBER IS NOT A CONGRESSIONAL DISTRICT, and this was the
  // largest single family in the implausible-spread audit — six pairs,
  // one shape:
  //
  //   "Who will win the Florida House of Representatives? - Republican"
  //   "Will the Republican Party win the FL-23 House seat?"
  //                                            0.877, priced 93 vs 6
  //
  // Nothing already here separated them. districtCodes rejects only on
  // MUTUAL disagreement and the chamber side names no district;
  // statesNamed sees FL on both; officesNamed sees "house" on both.
  //
  // Only unambiguous phrasings are claimed. The literal word "state"
  // before a chamber is one, and so is a state NAME before "house of
  // representatives" — a congressional race is written "U.S. House",
  // never "Florida House of Representatives". A bare "<State> Senate"
  // is deliberately NOT claimed: it reads as the state chamber to a
  // human and as the US Senate seat on Polymarket, and guessing would
  // cost correct Senate pairs.
  if (/\bstate (?:house|senate|assembly|legislature|election|parliament)\b/.test(t)) return "state";
  if (/\bstate elections\b|\blandtag\b|\bregional election/.test(t)) return "state";
  if (new RegExp(`\\b(?:${STATE_NAMES.join("|")}) house of representatives\\b`).test(t)) return "state";

  // The federal side of the same distinction. A district code, an
  // explicitly national chamber, or a congressional district.
  if (/\bu\.?s\.? (?:house|senate)\b|\bcongressional district\b|\bcongress\b/.test(t)) return "federal";
  if (districtCodes(title).length) return "federal";
  if (/\bbundestag\b|\bfederal election|\bgeneral election|\bnational election|\bparliamentary election/.test(t)) return "federal";
  return null;
}

// ── Who dissented, versus how many ──────────────────────────────
//
// Kalshi runs the FOMC dissent market as ONE QUESTION PER GOVERNOR —
// "Who will dissent at the September 2026 FOMC meeting? - Beth
// Hammack" — while Polymarket runs it as a HEADCOUNT: "Will no one
// dissent the September Fed decision?", "Will three people dissent".
// Those are different claims and were pairing at 0.813-0.839:
//
//   "- Beth Hammack"  vs  "Will no one dissent"      43 vs 10
//   "- Michael Barr"  vs  "Will three people dissent" 10 vs 30
//
// The "no one" case is the sharpest — a named individual dissenting
// and NOBODY dissenting are close to complements, and hasNegation does
// not fire because "no one" is not in its inverter list (deliberately:
// "no" is load-bearing in "no more than 3 cuts").
//
// Returns "count" for a headcount, "who" for a named subject, null for
// anything that is not about dissent at all — so this is inert outside
// the family, like econMetrics and departureModes.
const DISSENT_RE = /\bdissent(?:s|ed|ing)?\b/i;
const COUNT_WORDS = /\b(?:no one|nobody|none|zero|one|two|three|four|five|six|seven|eight|nine|ten|\d+)\s+(?:people|persons?|members?|officials?|governors?|voters?)?\s*(?:dissent|will dissent)/i;

export function dissentSubject(title) {
  const t = String(title || "");
  if (!DISSENT_RE.test(t)) return null;
  // A headcount, including the zero case Polymarket words as "no one".
  if (COUNT_WORDS.test(t)) return "count";
  if (/\bno one\b|\bnobody\b|\bhow many\b|\bnumber of\b/i.test(t)) return "count";
  // Kalshi's per-governor form: the outcome is a person, carried after
  // the em dash by the same convention every Kalshi bucket uses.
  if (/\bwho will dissent\b/i.test(t)) return "who";
  return null;
}

function dissentCompatible(titleA, titleB) {
  const a = dissentSubject(titleA), b = dissentSubject(titleB);
  // Rejects only on MUTUAL disagreement, like econMetrics: a title
  // this cannot read blocks nothing.
  return !(a && b && a !== b);
}

// ── A state legislature has no counterpart to pair with ─────────
//
// MEASURED 2026-09-09 across the ingested catalogue:
//
//   "state senate"     Kalshi 37   Polymarket 0
//   "state house"      Kalshi 24   Polymarket 0
//   "state assembly"   Kalshi  2   Polymarket 0
//
// Kalshi runs 63 state-legislature markets and Polymarket runs NONE.
// So every one of them is guaranteed to pair wrongly if it pairs at
// all, and they did — at 0.87 to 0.89:
//
//   "Who will win the Minnesota State Senate? - Democratic party"
//   "Will the Democrats win the Minnesota Senate race in 2026?"
//
// which is a state chamber against a US SENATE seat. governmentLevel
// cannot reject it: it fires only on MUTUAL disagreement and the
// Polymarket side names no level at all. That is the same asymmetry
// this file already records for statesNamed on the national-vs-state
// case — a rule that is right nearly everywhere and wrong on a title
// where naming nothing IS the claim.
//
// STRICT EQUALITY, both or neither, and the measurement is what makes
// that safe rather than aggressive: with zero Polymarket counterparts
// there is no correct pair for it to cost. It is also SELF-HEALING —
// if Polymarket ever lists state legislatures, both sides will say
// "state senate", the two answers will agree, and this stops firing
// without anyone editing it.
//
// A bare "<State> Senate" is deliberately NOT claimed. Kalshi writes
// "Who will win the Texas Senate?" for the state chamber while
// Polymarket writes "the Texas Senate race" for the federal seat, and
// nothing in either title separates them — that one needs the series
// ticker (KXSTATELEG), which the gate does not see.
const STATE_CHAMBER_RE = /\bstate (?:senate|house|assembly|legislature)\b/i;

export function namesStateChamber(title) {
  return STATE_CHAMBER_RE.test(String(title || ""));
}
...
function raceCompatible(titleA, titleB) {
  ...
  const lvlA = governmentLevel(titleA), lvlB = governmentLevel(titleB);
  if (lvlA && lvlB && lvlA !== lvlB) return false;

  if (hasPlaceholderSubject(titleA) !== hasPlaceholderSubject(titleB)) return false;
  if (!dissentCompatible(titleA, titleB)) return false;
  if (namesStateChamber(titleA) !== namesStateChamber(titleB)) return false;
  return true;
}
...
// ── Negation ────────────────────────────────────────────────────
//
// Embeddings put a claim and its exact complement almost on top of each
// other — "Zelenskyy and Putin meet before 2027" scored 0.908 against
// "Will Zelenskyy and Putin NOT meet before 2027?", and the two venues
// then priced them 7% and 92%, which is the complement working exactly
// as it should.
//
// Deliberately a SHORT list of inverters. "no" is excluded because it
// is load-bearing in phrases that do not negate the event at all — "no
// more than 3 cuts", "no earlier than June" — and one over-eager word
// here would reject a whole family of correct threshold pairs.
const NEGATION_RE = /\b(?:not|never|fails? to|failed to|won'?t|will not|doesn'?t|does not)\b/i;

export function hasNegation(title) {
  return NEGATION_RE.test(String(title || ""));
}

function negationCompatible(titleA, titleB) {
  // Reject only when exactly one side negates. Two negations, or none,
  // say nothing — the asymmetry is the whole signal.
  return hasNegation(titleA) === hasNegation(titleB);
}

// ── Opposing modifiers ──────────────────────────────────────────
//
// "Red wave in 2026?" against "Blue wave in 2026?" scored 0.913 and
// priced 6% against 77%. The titles share every word that carries
// meaning to an embedding except the one that reverses them.
//
// The rule needs BOTH halves of the asymmetry: A says red and not blue,
// B says blue and not red. A title mentioning both parties — which is
// most election markets — states no preference and must not be caught.
const OPPOSING = [
  [/\bred wave\b/i,        /\bblue wave\b/i],
  [/\brepublicans?\b|\bgop\b/i, /\bdemocrats?\b|\bdemocratic party\b/i],
  [/\bpass(?:es|ed)?\b/i,  /\bfails?\b|\brejected\b/i],
  [/\bincrease[sd]?\b|\bhike[sd]?\b/i, /\bdecrease[sd]?\b|\bcut[s]?\b/i],
];

function polarityCompatible(titleA, titleB) {
  for (const [x, y] of OPPOSING) {
    const ax = x.test(titleA), ay = y.test(titleA);
    const bx = x.test(titleB), by = y.test(titleB);
    // Each side must commit to one pole and only one.
    if (ax && !ay && by && !bx) return false;
    if (ay && !ax && bx && !by) return false;
  }
  return true;
}
...
//
// "Democrats win exactly 5 seats in the 2026 U.S. House of
// Representatives elections" against "How Many House Seats Will The
// Democrats Win In Arizona? — 5". Same number, same party, same
// chamber, utterly different claim.
//
// statesNamed() already existed and did not catch it, because the
// asymmetry rule rejects only on MUTUAL disagreement and the national
// side names no state. That rule is right nearly everywhere and wrong
// here: on a title that explicitly says "U.S." or "national", naming no
// state is not missing information, it is the claim.
const NATIONAL_RE = /\b(?:u\.?s\.?|united states|nationwide|national|federal)\b/i;

function scopeCompatible(titleA, titleB) {
  const sA = statesNamed(titleA), sB = statesNamed(titleB);
  const nationalA = NATIONAL_RE.test(titleA) && sA.length === 0;
  const nationalB = NATIONAL_RE.test(titleB) && sB.length === 0;
  if (nationalA && sB.length) return false;
  if (nationalB && sA.length) return false;
  return true;
}

// ── An event MONTH is not a deadline, and a bare one means the next
// occurrence ────────────────────────────────────────────────────
//
// "Will the Federal Reserve Hike rates by 25bps at their OCTOBER 2027
// meeting?" paired with "Fed Decision in OCTOBER - 25 bps Increase" at
// 0.815, a FULL YEAR apart. Every existing date rule was inert on it:
// neither title states a deadline at all — no "by", no "before" — so
// extractDeadline returns null on both sides and hasUnresolvedDeadline
// never fires. These are MEETING DATES, not cutoffs.
//
// A bare month means the SOONEST one still ahead: "Fed Decision in
// October", read in September 2026, is October 2026. So a side stating
// October 2027 is naming a different meeting.
//
// The tolerance is deliberately wide. This file already records that
// rejecting every bare date "would have killed four correct December
// pairs along with the August one" — Polymarket US drops years
// constantly. Six months cannot fire on any near-term ambiguity and
// still catches the twelve-month case that motivated it.
const MONTH_NAMES = ["january","february","march","april","may","june","july",
                     "august","september","october","november","december"];
const MONTH_RE = new RegExp(`\\b(${MONTH_NAMES.join("|")})\\b`, "i");

export function eventMonth(title) {
  const t = String(title || "");
  const m = MONTH_RE.exec(t);
  if (!m) return null;
  const month = MONTH_NAMES.indexOf(m[1].toLowerCase());
  // The year has to belong to THIS month, not to any four digits in
  // the title — "25bps" is not a year, and "2026 elections in October"
  // would otherwise read as October 2026 when it may not be.
  const near = t.slice(Math.max(0, m.index - 12), m.index + m[1].length + 12);
  const y = /\b(20\d{2})\b/.exec(near);
  return { month, year: y ? Number(y[1]) : null };
}

export function eventMonthsCompatible(titleA, titleB, { now = new Date(), toleranceMonths = 6 } = {}) {
  const a = eventMonth(titleA), b = eventMonth(titleB);
  if (!a || !b) return true;                 // inert unless BOTH name a month
  if (a.year != null && b.year != null) return true;  // deadlinesCompatible's job
  if (a.year == null && b.year == null) return true;  // two vague sides fall through

  const nowM = now.getUTCFullYear() * 12 + now.getUTCMonth();
  // Resolve the bare side to the soonest occurrence not already past.
  const resolve = side => {
    let m = now.getUTCFullYear() * 12 + side.month;
    if (m < nowM) m += 12;
    return m;
  };
  const abs = side => (side.year != null ? side.year * 12 + side.month : resolve(side));
  return Math.abs(abs(a) - abs(b)) < toleranceMonths;
}

export function scalarSignaturesCompatible(titleA, titleB, sportTag) {
  // Applies to every category: a negation inverts any claim, and an
  // inverted claim is the one thing guaranteed to price at the opposite
  // end of the book.
  if (!negationCompatible(titleA, titleB)) return false;
  if (!polarityCompatible(titleA, titleB)) return false;
  ...
  if (!eventMonthsCompatible(titleA, titleB)) return false;
  ...
  if (sportTag === "politics") {
    ...
    if (!scopeCompatible(titleA, titleB)) return false;
    // Losing an election is not retiring. See departureModes.
    if (!departuresCompatible(titleA, titleB)) return false;
  }
