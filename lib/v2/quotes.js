  ...
  for (const q of incoming) {
    if (changed(byId.get(q.listing_id), q)) {
      toInsert.push({
        ...
        // Depth rides along on rows we were writing anyway. It is
        // deliberately NOT part of `changed()`: size ticks constantly on
        // a liquid book, and treating it as a reason to write would undo
        // the whole point of write-on-change on a 500MB tier.
        bid_size: q.bid_size ?? null,
        ask_size: q.ask_size ?? null,
      });
    } else {
      skipped++;
    }
