import { authHeaders as supabaseAuth } from "../supabaseHeaders.js";
...
// Make a request with a specific credential rather than the default
// write key. Used by the health probe to check whether RLS actually
// refuses an anon write — asserting "RLS is on" without testing it is
// exactly the kind of assumption that stays wrong silently.
export async function restAs(key, path, options = {}) {
  try {
    ...
    return { status: 0, ok: false, body: JSON.stringify(describeError(err)) };
  }
}
...
// Real UPDATE (PostgREST PATCH), not upsert.
//
// Use this whenever you are setting a few columns on rows that already
// exist. `upsert` is INSERT ... ON CONFLICT, which validates NOT NULL
// columns against the attempted insert row *before* resolving the
// conflict — so a partial payload fails every row even though every row
// exists. Sending the full row avoids that, but is wrong here for a
// second reason: `listings` carries an embedding vector, and echoing
// those back would be megabytes of payload to change a few scalars.
export async function patchWhere(table, filter, patch) {
  const { error } = await rest(`${table}?${filter}`, {
    method: "PATCH",
