    ...
    results: {
      ...
      items: {
        ...
        properties: {
          ...
          // Nullable enums must be expressed with anyOf — declaring
          // type: ["string","null"] alongside `enum` is rejected by the
          // schema validator ("Enum value 'percent' does not match
          // declared type").
          unit: {
            anyOf: [
              { type: "string", enum: ["percent", "bps", "count", "usd", "other"] },
              { type: "null" },
            ],
          ...
          op: {
            anyOf: [
              { type: "string", enum: ["gt", "gte", "lt", "lte", "eq", "range", "winner"] },
              { type: "null" },
            ],
            description: "Comparator. 'range' when the claim is a bounded bucket.",
          },
          value: { anyOf: [{ type: "number" }, { type: "null" }], description: "Threshold for scalar comparators." },
          low: { anyOf: [{ type: "number" }, { type: "null" }], description: "Lower bound when op is 'range'." },
          high: { anyOf: [{ type: "number" }, { type: "null" }], description: "Upper bound when op is 'range'." },
          period_year: { anyOf: [{ type: "integer" }, { type: "null" }] },
          period_quarter: { anyOf: [{ type: "integer" }, { type: "null" }], description: "1-4, or null." },
          period_month: { anyOf: [{ type: "integer" }, { type: "null" }], description: "1-12, or null." },
          region: {
...
export function claimsCompatible(a, b, { minConfidence = 0.5 } = {}) {
  ...
  // Period granularity is NOT "missing data" — absence is information.
  // A Q4-2026 question and a full-year-2026 question measure different
  // things (quarterly growth vs annual growth), so if EITHER side names
  // a quarter, both must name the same one. Applying the usual
  // block-only-on-mutual-disagreement rule here let
  // "GDP > 2.5% in Q4 2026" match "GDP growth in 2026 > 2.5%" — a false
  // positive the regex gate correctly rejected, because its
  // periodsCompatible() already encoded this distinction.
  if ((a.period_quarter != null || b.period_quarter != null) &&
      a.period_quarter !== b.period_quarter) {
    return {
      compatible: false,
      reason: `period granularity ${a.period_quarter ? "Q" + a.period_quarter : "annual"} vs ${b.period_quarter ? "Q" + b.period_quarter : "annual"}`,
    };
  }
  if ((a.period_month != null || b.period_month != null) &&
      a.period_month !== b.period_month) {
    return {
      compatible: false,
      reason: `period granularity ${a.period_month ? "M" + a.period_month : "non-monthly"} vs ${b.period_month ? "M" + b.period_month : "non-monthly"}`,
    };
  }
