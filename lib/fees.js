...
export function bestArb(kalshi, poly) {
  ...
  const candidates = [
    { side: "kalshi-yes/poly-no", r: twoLegArb({ ...k, ask: kalshi.yesAsk }, { ...p, ask: poly.noAsk }) },
    { side: "poly-yes/kalshi-no", r: twoLegArb({ ...p, ask: poly.yesAsk }, { ...k, ask: kalshi.noAsk }) },
  ].filter(c => c.r != null);

  if (candidates.length === 0) return null;
  return candidates.reduce((best, c) => (c.r.total < best.r.total ? c : best));
}
