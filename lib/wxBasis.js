...
// A day is only scoreable if we actually WATCHED it.
//
// NWS serves at most `limit` observations per request, newest first,
// and most of these stations report every five minutes — so a 7-day
// request with limit=500 comes back holding TWO DAYS. That alone would
// be a smaller sample. What makes it a wrong answer is that the oldest
// day in any window is PARTIAL, and a partial day still produces a
// maximum: Denver's Sep 5 was read from three late-evening readings
// and scored 74F against a real high of 93F, which rendered as a 19F
// basis against The Weather Company rather than as missing data.
//
// So a day carries the count of distinct local HOURS it was observed
// in, and the caller refuses to score one that is short. Hours rather
// than readings, because a five-minute station can log 200 readings
// inside one hour and look complete.
export const MIN_HOURS_FOR_DAY = 20;
...
export function dailyExtremes(observations, tz) {
  ...
  for (const o of observations) {
    ...
    const f = cToF(raw);
    if (f == null) continue;
    const cur = byDate.get(d) || { date: d, highF: -Infinity, lowF: Infinity, n: 0, hours: new Set() };
    cur.highF = Math.max(cur.highF, f);
    cur.lowF = Math.min(cur.lowF, f);
    cur.n++;
    cur.hours.add(localHour(p.timestamp, tz));
    byDate.set(d, cur);
  }
  for (const v of byDate.values()) {
    if (!Number.isFinite(v.highF)) { v.highF = null; v.lowF = null; }
    v.hoursObserved = v.hours.size;
    v.complete = v.hoursObserved >= MIN_HOURS_FOR_DAY;
    delete v.hours;
  }
  return { byDate, rejected, noTemp };
}

// The local hour of an instant, as an integer 0-23.
