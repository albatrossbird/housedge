...
// not returned by the list endpoints while they are OPEN. /v1/events
// does page through `aec-` slugs, chronologically, from Oct 2025 — so a
// sweep of it looks like it disproves this — but
// /v1/events?closed=false returns 600+ open events and zero games. A
// game appears in the listing only once it can no longer be traded,
// while a direct lookup of aec-mlb-mil-nym-2026-08-27 returns a live
// book immediately.
//
...
// Polymarket US lists 71,378 open markets, MEASURED 2026-09-08 by
// paging /v1/markets?closed=false to exhaustion: 143 pages of 500.
//
// This ceiling has now been wrong twice, the same way both times. The
// original 20 x 200 = 4,000 covered a fifth of the venue, and I used
// that partial set to conclude the two venues do not overlap outside
// sports — politics showed 191 markets where there were 2,469. It was
// then raised to 60 x 500 = 30,000 against a venue of ~20,500, which
// was headroom at the time; the venue has since grown 3.5x and 30,000
// is 42% of it.
//
// So the number is not the fix — SAYING SO is. Exhausting the page cap
// used to end the loop indistinguishably from reading a short page,
// which is a truncation reported as a complete catalogue: the same
// shape as the embedded-titles read that re-bought politics embeddings
// every run. `truncated` is now returned and flows into the discovery
// job's `truncatedReads`, which fails the run.
//
// The cap is 400 pages (200,000 markets) so it is headroom rather than
// a limit, and it costs nothing: the loop early-stops on the first
// short page, so a 143-page catalogue issues 144 requests whatever the
// cap says. Page size is the maximum the gateway accepts.
export async function fetchPolymarketUs({ maxPages = 400, pageSize = 500, concurrency = 6 } = {}) {
  const out = [];
  const errors = [];
  let pagesRead = 0;
  // Only a SHORT PAGE proves the catalogue ended. Stopping for any
  // other reason — an error, or running out of pages — is a truncation
  // even though the rows already read are fine.
  ...
  for (let page = 0; page < maxPages && !sawShortPage; page += concurrency) {
    const wave = [];
    ...
    for (const r of results) {
      if (r.error) { errors.push(r.error); stop = true; continue; }
      pagesRead++;
      out.push(...r.rows);
      if (r.rows.length < pageSize) { sawShortPage = true; stop = true; }
    }
    if (stop) break;
  }

  // The gateway pages by OFFSET over a set that can include duplicates
  // across pages; dedupe on id rather than trusting the paging.
  // `duplicatesDropped` is reported because offset paging over an
  // order the server does not promise can also SKIP rows, and a
  // duplicate count climbing off zero is the visible half of that.
  const markets = [...new Map(out.map(m => [String(m.id), m])).values()];

  const truncated = !sawShortPage
    ? `polymarket.us: read ${pagesRead} pages x ${pageSize} and never reached the end of the catalogue — the fetch is TRUNCATED at ${markets.length} markets${errors.length ? ` (${errors[0]})` : ` (page cap ${maxPages})`}`
    : null;
  ...
  return {
    ...
    truncated,
    pagesRead,
    duplicatesDropped: out.length - markets.length,
  };
}

// market slug -> parent EVENT slug.
//
// A .us market carries no reference to its event: no eventSlug, no
// eventId, no ticker. The web route needs the event
// (/event/us-saa-q3-2026-10-29?marketSlug=gdpc-us-saa-q3-2026-10-29-gt2pt0),
// so without this every non-sports .us link is a 404.
//
// It cannot be derived from the market slug. The prefix is a per-family
// code of varying length (gdpc, cpc, enwc, nphc, ushsscc, pnwpc, rdc,
// vtc...) and the suffix is the outcome, and the event slug is not
// simply the middle: "cpc-btc-100k-10-31-2026" belongs to the event
// "btc-100k", which drops the date entirely.
//
// /v1/events?closed=false carries each event WITH its markets, so one
// paged pass builds the whole map. This is a lookup only — the market
// rows still come from /v1/markets, so a gap here costs a link, never a
// market.
// Page size 500, measured: the catalogue is 3,119 open events, which is
// seven requests and ~16s rather than thirty-one and ~23s. maxPages
// leaves an order of magnitude of headroom, because a short page is the
// only termination signal and silently truncating this map costs links
// with nothing to say so.
export async function fetchUsEventSlugs({ maxPages = 40, pageSize = 500 } = {}) {
  const map = new Map();
  for (let page = 0; page < maxPages; page++) {
    let events;
    try {
      const r = await usFetch(`${GATEWAY}/events?closed=false&limit=${pageSize}&offset=${page * pageSize}`);
      if (!r.ok) break;
      const data = await r.json();
    ...
    for (const e of events) {
      ...
      for (const m of e.markets || []) if (m && m.slug) map.set(String(m.slug), String(e.slug));
    }
...
// polymarket.us names college football "cfb" where our sport tag says
// "ncaaf". Getting this wrong builds aec-ncaaf-… slugs, every one of
// which 404s, and the league reads as "not listed on .us" — which is
// exactly what it did read as.
const US_LEAGUE = { ncaaf: "cfb" };
export function usLeagueFor(sportTag) {
  return US_LEAGUE[sportTag] || sportTag;
}

// The two teams a .us game slug names, in slug order — which is the
// order its prices come in. Digits are allowed because CFB codes carry
// them ("lcdbfc25").
export function usSlugCodes(slug) {
  const m = String(slug || "").match(/^aec-[a-z0-9]+-([a-z0-9]+)-([a-z0-9]+)-\d{4}-\d{2}-\d{2}$/);
  return m ? [m[1], m[2]] : null;
}

// Each marketSide carries its OWN price and its own team, which is the
// only alignment worth trusting here: on the MLB control the top-level
// arrays disagree outright — outcomes[0] is "Los Angeles Dodgers" while
// outcomePrices[0] is 0.3250, the NATIONALS' price.
//
// Which field holds the school name flips by league. MLB puts the full
// name in `description` ("Washington Nationals") and the mascot in
// `team.safeName` ("Nationals"); CFB does the reverse — `description`
// is "Spartans" and `team.safeName` is "San Jose State". So the caller
// gets both and matches against whichever agrees with Kalshi.
export function usSidesByCode(market) {
  const out = {};
  for (const side of market?.marketSides || []) {
    ...
    if (!abbr) continue;
    out[abbr] = {
      price: side.price != null ? Number(side.price) : null,
      names: [team.safeName, side.description, team.name, team.alias]
...
// THE FULL LADDER, which this venue does publish.
//
// This file, and CLAUDE.md, claimed for a long time that polymarket.us
// exposed only `bidDepth`/`askDepth` at the touch and no order book at
// all — so every US leg was skipped by the depth walk as `noPolyBook`,
// on the venue the site's default filter actually shows. Probed
// 2026-09-18 against a live MLB market: `/book` answers 200 with 21 bid
// levels and 23 offer levels, sizes included.
//
// The claim was never tested; it was inherited. Same lesson as the .us
// listing sweep and the truncated Kalshi econ fetch, for the third
// time: before concluding a venue does not publish something, ask it.
//
// THE ASK SIDE IS CALLED `offers`, not `asks`. Reading `asks` returns
// undefined, which `sortOffers` turns into an empty ladder — a book
// that is present and looks absent, which is very likely how this was
// written off in the first place.
export async function fetchUsBook(slug) {
  try {
    const r = await usFetch(`${GATEWAY}/markets/${encodeURIComponent(slug)}/book`);
    if (r.status === 404) return { notListed: true };
    if (!r.ok) return { error: `book ${r.status}` };

    const d = (await r.json()).marketData || {};
    // Levels arrive as { px: { value, currency }, qty } with both as
    // decimal STRINGS. Number() them here so no caller has to know.
    const levels = rows => (Array.isArray(rows) ? rows : [])
      .map(l => ({ price: Number(l?.px?.value), size: Number(l?.qty) }))
      .filter(l => isFinite(l.price) && isFinite(l.size) && l.size > 0);

    return { bids: levels(d.bids), asks: levels(d.offers), state: d.state || null };
  } catch (err) {
    return { error: err.message };
  }
}
...
export async function fetchUsGameMarket(league, codes, isoDate, opts = {}) {
  ...
  // Names and prices in slug order, for the leagues that need them.
  let sides = null;
  if (opts.nameKeyed) {
    const codesInSlug = usSlugCodes(slug);
    const byCode = usSidesByCode(market);
    if (codesInSlug && codesInSlug.every(c => byCode[c])) {
      sides = {
        // The FULLEST name each side offers. Kalshi says "San Jose
        // State", not "Spartans", so the school name has to win where
        // both are present.
        names: codesInSlug.map(c =>
          byCode[c].names.slice().sort((a, b) => b.length - a.length)[0]),
        prices: codesInSlug.map(c =>
          byCode[c].price == null ? null : String(byCode[c].price)),
      };
      // A side with no price is not a quote we can align, so drop the
      // whole rebuild rather than publish a half-aligned pair.
      if (sides.prices.some(p => p == null)) sides = null;
    }
  }
  ...
  return {
    ...
    // On a NAME-KEYED league the outcome cannot be resolved by code —
    // that is the whole reason those leagues exist — so both arrays are
    // rebuilt from marketSides in SLUG ORDER, where each side carries
    // its own price and its own team. The top-level arrays are not
    // merely differently ordered, they are misaligned: on the MLB
    // control outcomes[0] is the Dodgers while outcomePrices[0] is the
    // Nationals' price.
    //
    // Scoped to name-keyed leagues on purpose. MLB and NFL resolve
    // through the code path today and work; rewriting their arrays here
    // would be changing a load-bearing thing to fix a different one.
    ...(sides
      ? { outcomes: JSON.stringify(sides.names), outcome_prices: JSON.stringify(sides.prices) }
...
export function toMarketRow(market, eventSlug = null) {
  ...
  return {
    ...
    // The gateway calls it `description`, Kalshi calls it
    // rules_primary; one venue-neutral column so the card can put the
    // two resolution texts side by side and let the reader judge
    // whether the pair is really one claim.
    resolution: market.description || null,
    // The parent EVENT slug, which is the only thing that makes a
    // working web link — see fetchUsEventSlugs. Named event_ticker
    // because that column already exists and holds exactly this for
    // Kalshi.
    event_ticker: eventSlug || null,
    close_time: market.endDate || null,
    // NULL, not 0. The gateway publishes no volume field on a market at
    // all - not an empty one, absent entirely - so 0 is a number we made
    // up. It read as "nobody has traded this", which is a claim about
