...
import { fetchUsGameMarket, usLeagueFor, fetchPolymarketUs, fetchUsEventSlugs, toMarketRow, POLY_US_PLATFORM } from "./polymarketUs.js";
// Matching lives in lib/ so this route and scripts/match-category.mjs
// cannot drift — see the note at the top of that file.
import { matchNonSportsMarkets, assignGreedy, cosineSimilarity, explainKalshiRow } from "./matcher.js";
import { seriesTickerOf } from "./kalshiTicker.js";
import { buildSeriesGate, allowEmbed } from "./embedGate.js";
...
async function restFetch(path, options = {}) {
  ...
  try {
    const r = await fetch(url, {
      ...options,
      headers: {
        ...authHeaders(process.env.SUPABASE_ANON_KEY),
...
// A write that failed because the BATCH WAS TOO BIG, as opposed to
// because the data was wrong.
//
// 57014 is Postgres cancelling a statement that ran past
// statement_timeout, and it started appearing on econ once `markets`
// passed 133,000 rows: the same 50-row upsert does more index and TOAST
// work against a bigger table, so a batch size that was comfortable
// becomes marginal. 413 and 54000 are the payload-size equivalents.
//
// These are worth retrying SMALLER. Nothing else is — re-sending a
// 23502 NOT NULL violation in ten pieces fails ten times instead of
// once and buys nothing but latency.
export function isBatchSizeError(error) {
  const s = typeof error === "string" ? error : JSON.stringify(error || "");
  return /"httpStatus":(413|502|504)|57014|54000|statement timeout|too large|timeout/i.test(s);
}
...
// DEDUPED ON THE CONFLICT TARGET, because Postgres refuses a batch that
// names the same row twice:
//
//   21000  ON CONFLICT DO UPDATE command cannot affect row a second time
//
// and PostgREST fails the WHOLE batch, so 50 markets go unwritten. Econ
// hits this because its Polymarket side is five overlapping tags —
// federal reserve, interest rates, CPI, recession, GDP — and a Fed
// market carrying two of them is fetched twice and concatenated.
//
// Done here rather than at the call site because this is the choke
// point every write passes through, so no future caller has to remember.
// Keyed on `onConflict` rather than on "id" so the pairs table, whose
// target is (kalshi_id, polymarket_id), is covered by the same rule.
//
// The LAST occurrence wins: later sources in the concatenation are the
// more specific ones, and it matches what Postgres would have done had
// the rows arrived in separate statements.
...
// 50 -> 25 -> 12 -> 6 -> 3 -> 2 -> 1. Deep enough to reach a single
// row, so the floor is "name the row that cannot be written" rather
// than "give up on a batch".
const MAX_SPLIT_DEPTH = 6;

async function upsertRows(table, rowsIn, onConflict, batchSize = 50) {
  const errors = [];
  const warnings = [];
  let count = 0;
  ...
  const rows = dedupeOnConflict(rowsIn, onConflict);
  // Named, not silent. A non-zero count is a fact about the venues'
  // tagging, not a fault — but it should be visible rather than inferred
  // from a write that quietly lost fifty rows.
  const duplicateIdsDropped = rowsIn.length - rows.length;
  ...
  // Write one slice, and on a SIZE-SENSITIVE failure split it and write
  // the halves instead of losing all of them.
  //
  // The failure this exists for: econ came back green with
  // `marketsUpserted: 13563` and a 57014 statement timeout logged as a
  // warning. A 50-row batch had failed, so ~50 markets were not written
  // and nothing downstream could tell them from markets the venues had
  // delisted — which matters, because /api/prune deletes on exactly
  // that "not seen in 14 days" rule.
  //
  // Splitting rather than retrying at the same size is the point: the
  // batch timed out BECAUSE of its size, so a plain retry reproduces
  // it. Halving bottoms out at a single row, which either writes or
  // names the one row that cannot be written.
  const writeSlice = async (slice, depth = 0) => {
    if (!slice.length) return;
    const batch = alignKeys(slice);
    ...
    if (slice.length > 1 && depth < MAX_SPLIT_DEPTH && isBatchSizeError(error)) {
      splitRetries++;
    ...
    // Out of options. Say how many rows went unwritten, not just that
    // something went wrong: a shortfall nobody can size reads as noise.
    rowsFailed += slice.length;
    errors.push(`${slice.length} row(s) unwritten to ${table}: ${JSON.stringify(error)}`);
  };

  for (let i = 0; i < rows.length; i += batchSize) {
    await writeSlice(rows.slice(i, i + batchSize));
  }
  return { count, errors, warnings, duplicateIdsDropped, rowsFailed, splitRetries };
}
...
// ── Main handler ───────────────────────────────────────────────
// The discovery job itself, separated from the route.
//
// WHY IT MOVED. discover-markets.yml curled /api/embed twice per
// category plus /api/prune, and politics alone spends 86-109s of that.
// Measured 2026-09-07 that was the second largest consumer of the Hobby
// plan's 4 Fluid Active CPU-hours a month, after the price refresh —
// and like that job, nothing about reading two venues, calling Voyage
// and writing Supabase needs a serverless function.
//
// The route is KEPT so the endpoints stay hittable by hand (see the
// "Hitting the routes by hand" section in CLAUDE.md), and both callers
// run this same function, so the scheduled job and a manual poke cannot
// drift. Same shape as lib/refreshPrices.js.
//
// Returns { status, body } rather than writing a response, so the route
// maps it to HTTP and the runner reads the body directly.
export async function runEmbed(params = {}) {
  const q = params;
  ...
  const THRESHOLD = parseFloat(
    q.threshold || CATEGORY_THRESHOLDS[sport] || "0.78"
  );
  ...
  // 300s ceiling at all.
  const explainId = q.explain;
  if (explainId) {
    const { data: kRows, error: kErr } = await supabase
      .from("markets").select("id, title, sport_tag, embedding_v")
      .eq("id", explainId).limit(1);
    if (kErr) return __r(500, { error: `kalshi read failed: ${kErr.message}` });
    const km = (kRows || [])[0];
    if (!km) return __r(404, { error: `no market with id ${explainId}` });
    if (!km.embedding_v) {
      return __r(200, {
    ...
    const polyRows = await fetchAllRows(() => supabase
      ...
      { errors: polyErrors });
    const limit = Math.min(parseInt(q.top, 10) || 10, 50);
    return __r(200, {
  ...
  try {
    if (matchOnly) {
      ...
      if (sportFilter && SPORTS_TAGS.has(sportFilter)) {
        ...
        // guessed at.
        const topK = Math.min(Math.max(parseInt(q.topk || "10", 10) || 10, 1), 100);
    ...
    if (SPORTS_TAGS.has(sport)) {
      ...
      // A truncated .us fetch is a WRONG answer, not a short one: every
      // market past the cut reads as "this venue does not list it",
      // which is the mistake that once had this file concluding the two
      // venues do not overlap outside sports. Route it into
      // truncatedReads so the run FAILS rather than reporting a smaller
      // catalogue as a complete one.
      if (us.truncated) truncatedReads.push(us.truncated);
      usGames = {
        markets: rows,
        diagnostics: {
          fetched: us.markets.length,
          forCategory: rows.length,
          pagesRead: us.pagesRead ?? null,
          duplicatesDropped: us.duplicatesDropped ?? null,
          truncated: us.truncated || null,
          errors: us.errors.slice(0, 3),
        },
      };
    }
    ...
    // from then on.
    const gateOff = q.reprobe === "1" || force;
    ...
    const embedLimit = Math.max(1, parseInt(q.embedLimit, 10) || EMBED_BATCH_LIMIT);
    ...
    if (fetchOnly) {
      ...
      return __r(200, {
        ...
        writes: {
          ...
          // Rows the venues handed us twice. Econ's Polymarket side is
          // five overlapping tags, so this is expected and non-zero
          // there; it is reported because the alternative is a batch
          // that silently loses fifty rows to a 21000.
          duplicateIdsDropped: (marketsUpsert.duplicateIdsDropped || 0) +
                               (embeddingUpsert.duplicateIdsDropped || 0),
          // How many rows went UNWRITTEN, sized rather than implied.
          // A market that failed to write is indistinguishable
          // downstream from one the venues delisted, and /api/prune
          // deletes on exactly that "not seen in 14 days" rule — so
          // this is an error, not a note.
          rowsFailed: (marketsUpsert.rowsFailed || 0) + (embeddingUpsert.rowsFailed || 0),
          // Batches that had to be split to get through. Non-zero is
          // the fix working; a number that climbs run over run says
          // batchSize is now too big for the table's size.
          splitRetries: (marketsUpsert.splitRetries || 0) + (embeddingUpsert.splitRetries || 0),
        },
    ...
    return __r(200, {
      ...
      writes: {
        ...
        rowsFailed: (marketsUpsert.rowsFailed || 0) + (embeddingUpsert.rowsFailed || 0) +
                    (pairsUpsert.rowsFailed || 0),
        splitRetries: (marketsUpsert.splitRetries || 0) + (embeddingUpsert.splitRetries || 0) +
                      (pairsUpsert.splitRetries || 0),
      },
      ...(matchDiagnostics ? { matchDiagnostics } : {}),
    });
  } catch (err) {
