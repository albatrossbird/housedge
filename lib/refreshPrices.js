...
import { fetchUsBbo } from "./polymarketUs.js";
...
async function applyUpdates(updates) {
  ...
  for (let i = 0; i < updates.length; i += CHUNK) {
    ...
    if (error && isMissingColumnError(error)) {
      ...
      attempt = merged.map(stripBookColumns);
      ({ error } = await send(attempt));
    }

    if (!error) { updated += merged.length; continue; }

    // A WHOLE CHUNK LOST FOR ONE BAD ROW, EVERY RUN.
    //
    // Measured across six consecutive runs: exactly one chunk of ~48
    // failed on `57014 canceling statement due to statement timeout`,
    // every time. Random IO pressure would vary; a constant of one says
    // the same rows fail each run, so ~100 markets were never
    // refreshing and their prices were frozen — and the error said only
    // "upsert: {...}", naming none of them, so nothing could tell you
    // which.
    //
    // Halving isolates it: a transient timeout passes on the retry, and
    // a genuinely pathological row is narrowed to ONE id while the
    // other 99 are written. Same shape as fetchAllRows halving its page
    // on a payload-size failure, for the same reason.
    const split = await sendSplitting(attempt, send);
    updated += split.written;
    if (split.failed.length) {
      // Named, because a frozen market nobody can identify is the
      // stale-price bug this job exists to prevent.
      errors.push(`upsert: ${split.failed.length} row(s) failed after splitting `
        + `[${split.failed.slice(0, 20).join(", ")}${split.failed.length > 20 ? ", ..." : ""}] `
        + JSON.stringify(split.error));
    }
    if (split.written) {
      warnings.push(`upsert recovered ${split.written} row(s) by splitting a failed chunk`);
    }
  }
...
// Write `rows`, halving on failure until either everything lands or the
// offending rows are isolated to single ids. Depth is bounded by the
// halving itself (100 rows reaches 1 in seven splits).
async function sendSplitting(rows, send) {
  const { error } = await send(rows);
  if (!error) return { written: rows.length, failed: [], error: null };
  if (rows.length === 1) {
    return { written: 0, failed: [rows[0].id], error };
  }
...
export const ON_DEMAND_FLOOR_SECONDS = 180;

// The price refresh job itself, separated from the route that used to
// be its only caller.
//
// WHY IT MOVED. /api/refresh ran on Vercel every ~2 hours from GitHub
// Actions and did all the venue reading there: ~8-12 runs a day at ~90
// seconds each. Measured 2026-09-07 that was the dominant consumer of
// the Hobby plan's 4 Fluid Active CPU-hours a MONTH, at 3 of 4 used —
// spent almost entirely on cron traffic hitting our own API, with
// essentially no visitors.
//
// Nothing about reading two public venues and writing Supabase needs a
// serverless function, so the SCHEDULED sweep now runs in the Actions
// runner (free and unmetered on a public repo), the same way
// match-markets.yml and the m15 jobs already do.
//
// THE ROUTE STAYS, and that is the point of extracting rather than
// moving. pages/index.js calls /api/refresh on every visit where the
// prices on screen are over three minutes old, and that on-demand path
// is what keeps freshness at ~3 minutes instead of the 45 minutes to
// 3.5 hours the throttled cron manages. Both callers run THIS function,
// so the two can never drift.
//
// `ifStale` is resolved by the CALLER, because the policy differs by
// caller and the job should not care: the route floors an
// unauthenticated request at ON_DEMAND_FLOOR_SECONDS, while the
// scheduled runner passes 0 and always reads the venues.
export async function runRefresh({ ifStale = null, cooldownFloored = null } = {}) {
      ...
      if (!error && data && data.length) {
        ...
        if (ageSeconds < ifStale) {
          return {
            ...
            ...(cooldownFloored ? { cooldownFloored } : {}),
          };
        }
      }
    ...
    // Read through restFetch rather than supabase-js, which was this
    // file's ONLY use of that package — and the reason the first
    // runner-based run died with ERR_MODULE_NOT_FOUND, since a runner
    // has no node_modules. Every other read here already used the raw
    // REST helper, which also preserves the HTTP status and body that
    // supabase-js flattens to a bare "TypeError: fetch failed".
    //
    // KEYSET, NOT OFFSET. `.range(from, from + 999)` was LIMIT/OFFSET
    // with no ORDER BY — the same defect that let the embedding read
    // skip rows and buy them again, and that made prune O(n^2) until it
    // timed out. `id` is selected only to give the pager a key; the
    // protection set is built from the other two columns.
    const pairRows = [];
    const pairErrors = [];
    let lastId = null;
    for (let page = 0; page < 200; page++) {
      const q = "pairs?select=id,kalshi_id,polymarket_id&order=id.asc&limit=1000" +
                (lastId != null ? `&id=gt.${encodeURIComponent(lastId)}` : "");
      const { data, error } = await restFetch(q);
      if (error) { pairErrors.push(JSON.stringify(error).slice(0, 200)); break; }
      if (!data || data.length === 0) break;
      pairRows.push(...data);
      if (data.length < 1000) break;
      lastId = data[data.length - 1].id;
      if (lastId == null) { pairErrors.push("pairs read: no id on row, cannot page"); break; }
    }
    // A failed pairs read is not an empty pairs table. Refreshing
    // nothing because the read broke is exactly the silent no-op this
    // job keeps being rewritten to stop.
    if (pairErrors.length && pairRows.length === 0) {
      throw new Error(`could not read pairs: ${pairErrors[0]}`);
    }
    ...
    return {
      ...
      ...(pairErrors.length ? { pairReadErrors: pairErrors.slice(0, 3) } : {}),
