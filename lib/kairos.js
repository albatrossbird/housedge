...
// The candle covering the minute that ENDS at t. bucket_start is the
// start of the bucket, so the minute ending at t is stamped t-60.
export function candleEndingAt(candles, tSecs) {
  const want = (tSecs - 60) * 1000;
  for (const c of candles || []) {
    const b = Date.parse(c.bucket_start);
    if (Number.isFinite(b) && Math.abs(b - want) < 1000) return c;
  }
  return null;
}
