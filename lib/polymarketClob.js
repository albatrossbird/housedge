...
// polymarket id -> clobTokenIds[].
//
// KEYED ON THE MARKET ID, NOT THE SLUG. `markets.slug` holds the EVENT
// slug for polymarket.com — "gdp-growth-in-2026" is one event carrying
// every GDP bucket — and gamma's `?slug=` filter wants a MARKET slug,
// so a batch of them returns ZERO rows. Measured: 4 slugs taken off
// live profitable legs returned 0 markets, which is why the first
// version of this checked 2 of 45 legs and reported no error, having
// asked a question the venue could not answer.
//
// `markets.id` for a Polymarket row IS gamma's market id, so it needs
// no lookup and no second convention.
//
// Three things about this endpoint, all measured 2026-09-08:
//
//   * `id=` must be a REPEATED key; a comma-joined list is ignored.
//   * A NON-NUMERIC id 422s the WHOLE batch, so ids are filtered to
//     digits rather than trusted — one Kalshi ticker slipping in would
//     cost every leg in the chunk.
//   * `closed=true` returns only CLOSED markets (it is a filter, not an
//     include-flag) and came back with 0 of 6 live ids. It is not sent.
//
// `limit` is explicit because gamma applies a default of 20 to a batch
// of any size and returns the truncated list with a 200.
export async function fetchTokenIdsById(ids, { fetchImpl = fetch } = {}) {
  const out = new Map();
  const errors = [];
  const all = [...new Set(ids.filter(Boolean).map(String))];
  const wanted = all.filter(id => /^\d+$/.test(id));
  const skipped = all.length - wanted.length;
  if (skipped) errors.push(`${skipped} non-numeric polymarket id(s) skipped — a single one 422s the whole batch`);
  ...
  for (const part of chunk(wanted, 50)) {
    ...
    try {
      ...
      for (const m of got) {
        ...
        if (Array.isArray(tokenIds) && tokenIds.length) out.set(String(m.id), tokenIds.map(String));
      }
      // Asked-for against came-back, because the whole failure this
      // module keeps meeting is a short answer served as a complete one.
      if (got.length < part.length) {
        errors.push(`gamma returned ${got.length} of ${part.length} ids asked`);
      }
    } catch (err) {
      errors.push(`gamma fetch: ${err.message}`);
    }
  }
  return { tokensById: out, errors, asked: wanted.length };
}
...
export async function fetchClobBooks(tokenIds, { fetchImpl = fetch } = {}) {
  ...
  for (const part of chunk(wanted, CLOB_BOOKS_BATCH)) {
    try {
      ...
      const rows = await r.json();
      const got = Array.isArray(rows) ? rows : [];
      // THE TOUCH AND THE BOOK, not one or the other.
      //
      // This stored only touchOf(b) — four numbers — and a later caller
      // that wanted to walk the ladder read `.asks` off it, got
      // undefined, and reported an empty book for 46 of 46 profitable
      // legs. The touch is what most callers want and stays at the top
      // level so none of them change; `book` carries the levels for the
      // ones that need depth rather than a price.
      for (const b of got) if (b?.asset_id) books.set(String(b.asset_id), { ...touchOf(b), book: b });
